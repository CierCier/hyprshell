...
> Source: https://nix.dev/guides/recipes/dependency-management.html
> Upstream: https://github.com/nixos/nix.dev (`source/guides/recipes/dependency-management.md`)

The Nix language describes dependencies between files that Nix manages.
Nix expressions themselves can depend on remote sources, and there are multiple ways to specify their origin, as shown in [Towards reproducibility: pinning Nixpkgs](https://nix.dev/tutorials/first-steps/towards-reproducibility-pinning-nixpkgs.html).
...
This pattern allows overriding remote sources programmatically (see Overriding sources below).
...
See [Dependencies in the development shell](./sharing-dependencies.md) for details, and note that here you have to pass an empty attribute set to the imported expression, since `default.nix` now contains a function.
...
To use the [v3 command line](https://nix.dev/manual/nix/stable/command-ref/new-cli/nix.html) and run programs from dependencies
exposing packages through a [flake](https://nix.dev/concepts/flakes), like `nix run nixpkgs#hello`,
...
- For more details and examples of the different ways to specify remote sources, see [Towards reproducibility: pinning Nixpkgs](https://nix.dev/tutorials/first-steps/towards-reproducibility-pinning-nixpkgs.html).
