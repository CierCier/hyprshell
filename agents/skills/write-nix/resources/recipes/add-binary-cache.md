...
> Source: https://nix.dev/guides/recipes/add-binary-cache.html
> Upstream: https://github.com/nixos/nix.dev (`source/guides/recipes/add-binary-cache.md`)
...
> **Warning:** Nix will accept any requested store object signed with private keys corresponding to the configured public keys.
...
> **NixOS:** On NixOS, Nix is configured through the [`nix.settings`](https://search.nixos.org/options?show=nix.settings) option:
>
> ```nix
> { ... }: {
>   nix.settings = {
...
- Follow the tutorial to [set up your own HTTP binary cache](https://nix.dev/tutorials/nixos/binary-cache-setup.html)

> **Tip:** Use [remote build machines](https://nix.dev/tutorials/nixos/distributed-build-setup.html) as preferred binary caches to reduce your external traffic.
