...
> Source: https://nix.dev/tutorials/nixos/distributed-builds-setup.html
> Upstream: https://github.com/nixos/nix.dev (`source/tutorials/nixos/distributed-builds-setup.md`)
> Authors: tfc; Editors: fricklerhandwerk
...
- Familiarity with the [Nix language](https://nix.dev/tutorials/nix-language.html)
- Familiarity with the [NixOS module system](https://nix.dev/tutorials/module-system/)

- A *local machine* (example hostname: `localmachine`)

  The computer [with Nix installed](https://nix.dev/install-nix.html) that distributes builds to other machines.

- A *remote machine* (example hostname: `remotemachine`)

  A computer running NixOS that accepts build jobs from the *local machine*.
  Follow [Provisioning remote machines via SSH](https://nix.dev/tutorials/nixos/provisioning-remote-machines.html) to set up a remote NixOS system.
...
> **Note:**
If your *local machine* runs NixOS, skip this section and [configure Nix through module options](https://nix.dev/tutorials/nixos/distributed-builds-setup.html).
...
- A list of [supported system features](https://nix.dev/manual/nix/2.23/command-ref/conf-file#conf-system-features)
  ...
  This particular list must be specified to delegate building compilers and running [NixOS VM tests](https://nix.dev/tutorials/nixos/integration-testing-using-virtual-machines.html) to remote machines.
...
- [Configure Nix to use a custom binary cache](https://nix.dev/guides/recipes/add-binary-cache.html) on each remote builder
- [Setting up post-build hooks](https://nix.dev/guides/recipes/post-build-hook.html) to upload store objects to a binary cache

To set up multiple builders, repeat the instructions in the [Setting up distributed builds](https://nix.dev/tutorials/nixos/distributed-builds-setup.html) section for each remote builder.
Add all new remote builders to the `nix.buildMachines` attribute shown in the [Setting up distributed builds](https://nix.dev/tutorials/nixos/distributed-builds-setup.html) section.

> **Tip:**
Configure remote build machines to [host a binary cache](https://nix.dev/tutorials/nixos/binary-cache-setup.html) and use them as [preferred binary caches](https://nix.dev/guides/recipes/add-binary-cache.html) to reduce your external traffic.
