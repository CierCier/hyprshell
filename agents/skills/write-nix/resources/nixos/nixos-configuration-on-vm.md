...
> Source: https://nix.dev/tutorials/nixos/nixos-configuration-on-vm.html
> Upstream: https://github.com/nixos/nix.dev (`source/tutorials/nixos/nixos-configuration-on-vm.md`)
> Authors: olafklingt, domenkozar; Editors: fricklerhandwerk
...
- Basic knowledge of the [Nix language](https://nix.dev/tutorials/nix-language.html)
...
For a thorough treatment of the module system, check the [NixOS module system deep dive](https://nix.dev/tutorials/module-system/deep-dive.html) tutorial.
...
If you prefer, you can skip ahead to the [sample configuration](https://nix.dev/tutorials/nixos/nixos-configuration-on-vm.html) section.
...
- The positional argument to [`nix-build`](https://nix.dev/manual/nix/stable/command-ref/nix-build.html) is a path to the derivation to be built.
  That path can be obtained from [a Nix expression that evaluates to a derivation](https://nix.dev/tutorials/nix-language.html).
  ...
  Therefore we use the [lookup path](https://nix.dev/tutorials/nix-language.html) `<nixpkgs/nixos>`.
...
- The [`-I` option](https://nix.dev/manual/nix/stable/command-ref/opt-common.html#opt-I) prepends entries to the search path.
  ...
  Here we set `nixpkgs` to refer to a [specific version of Nixpkgs](https://nix.dev/tutorials/first-steps/towards-reproducibility-pinning-nixpkgs.html) and set `nix-config` to the `configuration.nix` file in the current directory.
</details>
...
- [Building a bootable ISO image](https://nix.dev/tutorials/nixos/building-bootable-iso-image.html)
